...
class ControllerExtensionFeedYamodule extends Controller {
		...
		switch($this->request->post['type_data'])
		{
			case 'kassa':
				...
				if(isset($this->request->post['ya_kassa_active']) && $this->request->post['ya_kassa_active'] == 1){
					$testUrl = $this->url->link('extension/payment/yamodule/test', 'token=' . $this->session->data['token'], true);
	...
	public function initForm($array)
	{
		...
		$data['ya_metrika_callback'] = $this->url->link('extension/feed/yamodule/preparem', 'token='.$this->session->data['token'], true);
		...
		$data['ya_pokupki_callback'] = $this->url->link('extension/feed/yamodule/preparep', 'token='.$this->session->data['token'], true);
	...
	public function gocurl($type, $post)
	{
		...
		if ($status == 200) {
			if (!empty($data->access_token))
			{
				...
				$this->response->redirect($this->url->link('extension/feed/yamodule', 'token=' . $this->session->data['token'], true));
			}
		}

		$this->response->redirect($this->url->link('extension/feed/yamodule', 'err='.$data->error_description.'&token=' . $this->session->data['token'], true));
	}
	...
	public function index()
	{
		...
		$data['txt_mws_connect'] = sprintf($this->language->get('txt_mws_connect'),$this->url->link('yamodule/mws/csr', 'token=' . $this->session->data['token'], true));
		$data['success_mws_alert'] = sprintf($this->language->get('success_mws_alert'), $this->url->link('sale/order', 'token=' . $this->session->data['token'], true), $this->url->link('extension/feed/yamodule', 'token=' . $this->session->data['token'], true));
		...
		$data['url_mws_gen'] = $this->url->link('yamodule/mws/generate', 'token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			'href'      => $this->url->link('extension/feed/yamodule', 'token=' . $this->session->data['token'], true),
			'text'      => $this->language->get('heading_title_ya'),
			'separator' => ' :: '
		);
		$this->tabMws($data);
		$data['action'] = $this->url->link('extension/feed/yamodule', 'token=' . $this->session->data['token'], true);

<?php 
class ControllerExtensionTotalRpcodtotal2 extends Controller { 
	private $error = array(); 
	private $data = array(); 
	 
	public function index() { 
		$this->load->language('extension/total/rpcodtotal2');
		...
		$this->data['button_cancel'] = $this->language->get('button_cancel');
 		...
 		if (isset($this->error['warning'])) {
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/total/rpcodtotal2', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('extension/total/rpcodtotal2', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('extension/total/rpcodtotal2', $this->data));
	}

	private function validate() {
		if (!$this->user->hasPermission('modify', 'extension/total/rpcodtotal2')) {

...
class ControllerExtensionPaymentrpcod2ems extends Controller {
	...
	public function index() { 
		$this->language->load('extension/payment/rpcod2ems');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			{
				$this->response->redirect( $this->url->link('extension/payment/rpcod2ems', 'token=' . $this->session->data['token'], 'SSL') );
			}
			else
			{
		...
		$russianpost2_order_filters = $this->model_extension_shipping_russianpost2->getFilters('order');
		...
		$this->data['shipping_mod'] = $this->url->link('extension/shipping/russianpost', 'token=' . $this->session->data['token'], 'SSL');

		$this->data['action_shipping'] = $this->url->link('extension/shipping/russianpost', 'token=' . $this->session->data['token'], 'SSL');
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/payment/rpcod2ems', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('extension/payment/rpcod2ems', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('extension/payment/rpcod2ems', $this->data));
	}
	
	
	private function validate() 
	{
		if (!$this->user->hasPermission('modify', 'extension/payment/rpcod2ems')) {
